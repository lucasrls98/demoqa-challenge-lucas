...
| CI only: modal a11y scan failed `color-contrast` on `#closeSmallModal` (3.52:1) on all 3 attempts, but passed locally | Retries could not help because the failure was deterministic. The colors measured were not the button’s resting style, most likely an in-flight CSS transition on the slower runner. The a11y command now disables CSS transitions and animations and waits for zero running animations before scanning. |
